...
int main(void)
{
	...
	for (capacity = 0; arts[capacity]; capacity++) {
	}
	...
	for (int i = 0; i < capacity; i++) {
		if (dictInsert(dict, arts[i]->name, arts[i]) == -1)
			return (-1);
	}
	for (int i = 0; i < capacity; i++) {
		printf("%s => %d\n", arts[i]->name, searchPrice(dict, arts[i]->name));
	}
	printf("%s => %d\n", "Empty string", searchPrice(dict, ""));
	printf("%s => %d\n", "Non-existant", searchPrice(dict, "gqighqig"));
	printf("%s => %d\n", "Non-existant", searchPrice(dict, "United Silk-Weaving Mills, Krefeld: conference room furnishings (Elevations, sections, and plans. Table for stenography and telephone table.)"));
	return (0);
}
